...
`new`, `setup`, `pop`, `ask`, `browse`, and `help` are reserved as the first word. `llote pop` does not write a note that says "pop" — it offers to delete your last one (y/N, default no). To log a reserved word, pass it as the second argument after a file: `llote log.llote pop`.
